...
async function checkForNonEditableScenario(payload) {
  ...
  try {
     ...
     * in "Not Started" state. If simulations exist and none is "Not Started",
     * push a validation error.
     */
    const hasNotStartedSimulation = simulations.some(
      (item) => item.simulation_status === SIMULATION_STATUSES.NOT_STARTED
    );
    if (simulations.length > 0 && !hasNotStartedSimulation) {
